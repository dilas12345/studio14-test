...
const History = ({ cardHistory }) => {
	const getHistoryItemTitle = item => {
		...
		switch (item.type) {
			...
			case 'prepaidCard': {
				typeTitle = 'Card recharge';
				break;
			}
			case 'withdrawCard': {
				typeTitle = 'Card transfer';
				break;
			}
			default: {
				typeTitle = 'Operation';
	...
	const getContent = list => {
		...
		return content.length === 0
			? <HistoryContent><HistoryEmpty>Operation history is empty</HistoryEmpty></HistoryContent>
			: <HistoryContent>{content}</HistoryContent>;
	};

	return (
		<HistoryLayout>
			<HistoryTitle>Today</HistoryTitle>
